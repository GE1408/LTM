...
public class TongConLonNhat {
    public static void main(String[] args) throws Exception {
        String studentCode = "B22DCVT090";
        String qCode = "cZCYfqMK";
        ...
        for (int i = 1; i <= n; i++) {
            ...
            long currentSum = pref[i] - pref[deque.peekFirst()];
            if (currentSum > maxSum) {
                maxSum = currentSum;
            }
            
            while (!deque.isEmpty() && pref[deque.peekLast()] >= pref[i]) {
                deque.pollLast();
            }
            deque.addLast(i);
        }
...
//    (mảng số nguyên) và `k`.
// c. Tìm tổng lớn nhất của mọi đoạn con liên tiếp có
//    độ dài không vượt quá `k` bằng prefix sum và deque.
// d. Gửi POST /api/rest/data/submit với body JSON gồm
//    studentCode, qCode, requestId và answer.
// e. Ví dụ: nếu `values=[-1,4,2,-3,5]` và `k=3`
//    thì `answer` là `7`.